...
def run_nmap_scan(network, nmap_output_file):
    try:
        result = subprocess.run([
            "nmap", "-sn", network
        ], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        with open(nmap_output_file, "a") as f:
            f.write(f"[+] Nmap taraması tamamlandı: {network}\nÇıktı:\n{result.stdout}\n")
        print(f"[+] Nmap taraması tamamlandı: {network}\nÇıktı dosyaya yazıldı.")
...
def scan_gateways(output_file, progress_file, nmap_output_file):
        ...
        for subnet_16 in base_network.subnets(new_prefix=16):
            for subnet_24 in subnet_16.subnets(new_prefix=24):
                ...
                for gateway_ip in gateway_ips:
                    ...
                    scan_futures[executor.submit(run_nmap_scan, gateway_ip, nmap_output_file)] = gateway_ip
    ...
    active_vlans = parse_nmap_output(nmap_output_file)

    # Save active VLANs
    print(f"[+] Aktif VLAN'lar dosyaya yazılıyor: {output_file}")
...
if __name__ == "__main__":
    ...
    scan_gateways(args.output, args.progress, args.nmap_output)
